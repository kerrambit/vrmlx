...
namespace vrml_proc::traversor::handler::LODHandler {
  /**
   ...
   *
   * @todo  New type `Float32Array` have to be implemented in parser, so full logic in this handler can be implemented.
   * Now we ignore `range` field and just resolve all VRML nodes in `level` array.
   */
  template <vrml_proc::core::contract::ConversionContextable ConversionContext>
  TraversorResult<ConversionContext> Handle(HandlerParameters<ConversionContext> params) {  //
    ...
    for (const auto& child :
        params.nodeView->template GetField<std::vector<std::reference_wrapper<const VrmlNode>>>("level")) {
